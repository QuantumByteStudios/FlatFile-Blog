...
$detected_scheme = $is_https ? 'https://' : 'http://';
$detected_host = $_SERVER['HTTP_HOST'] ?? 'localhost';
$detected_script_dir = rtrim(dirname($_SERVER['SCRIPT_NAME'] ?? ''), '/\\');
$detected_base_url = $detected_scheme . $detected_host . ($detected_script_dir ? $detected_script_dir : '');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !isset($_POST['action'])) {
    ...
    $interface_mode = trim($_POST['interface_mode'] ?? 'classic');
    ...
    if (!in_array($interface_mode, ['classic', 'custom'], true)) {
        addError('Invalid interface mode selected');
    }
...
define('INTERFACE_MODE', '" . addslashes($interface_mode) . "'); // classic | custom
...
?>";
        ...
        if (!is_path_writable_for_create('content/settings.json')) {
            ...
            addStep("Selected interface mode: " . $interface_mode);
        } else {
...
<body>
                            ...
                            <form method="POST" class="needs-validation" novalidate>
                                ...
                                <div class="mb-4">
                                    <label class="form-label">
                                        <i class="bi bi-display me-2"></i>Interface Mode
                                    </label>
                                    <div class="border rounded p-3 mb-2">
                                        <div class="form-check mb-2">
                                            <input class="form-check-input" type="radio" name="interface_mode"
                                                id="interface_mode_classic" value="classic" checked>
                                            <label class="form-check-label" for="interface_mode_classic">
                                                <strong>Use the classic interface</strong>
                                            </label>
                                        </div>
                                        <div class="small text-muted">
                                            Includes built-in public pages (`index.php` + `post.php`) and standard RSS XML output.
                                        </div>
                                    </div>
                                    <div class="border rounded p-3">
                                        <div class="form-check mb-2">
                                            <input class="form-check-input" type="radio" name="interface_mode"
                                                id="interface_mode_custom" value="custom">
                                            <label class="form-check-label" for="interface_mode_custom">
                                                <strong>Use the custom interface</strong>
                                            </label>
                                        </div>
                                        <div class="small text-muted">
                                            Disables built-in public UI pages. `rss.php` returns JSON only so you can build your own frontend.
                                        </div>
                                    </div>
                                </div>
                                    ...
                                    <div class="col-md-6">
                                        <div class="mb-3">
                                            ...
                                            <input type="url" class="form-control" id="base_url" name="base_url"
                                                value="<?php echo htmlspecialchars($detected_base_url, ENT_QUOTES, 'UTF-8'); ?>" required>
    ...
    <script src="<?php echo htmlspecialchars($detected_base_url, ENT_QUOTES, 'UTF-8'); ?>/assets/js/install.js"></script>
</body>
